...
from .unet_blocks import CBR, BasicBlock, DownSample, DANetHead


class DAResUNet(nn.Module):
    def __init__(self, seg_classes=2, k=16, input_channel=1, psp=False):
        super(DAResUNet, self).__init__()

        self.layer0 = CBR(input_channel, k, 7, 1)
        self.class0 = nn.Sequential(
            BasicBlock(k + 2 * k, 2 * k),
            nn.Conv3d(2 * k, seg_classes, kernel_size=1, bias=False)
        )
